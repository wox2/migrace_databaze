...
zlepšující krok (improvement). V průběhu vývoje je možné aplikovat operaci ze
stejné třídy s jinými parametry. Proto algoritmus hledá nejlepší zlepšující krok
pro danou třídu operací viz řádek \ref{algo:state:improvement}. 
...
algoritmus vrátí pro operaci hodnotu $-\infty$. Pro zlepšující krok
getImprovement(c_{op}, A, T) platí \eqref{eq:improvement}.

\begin{equation}distance(M_A, M_T) = distance(M_X, M_T) + getImprovement(op, A,
