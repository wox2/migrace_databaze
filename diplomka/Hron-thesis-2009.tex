...
\subsection{Seznam aplikačních operací}\label{ORMo}
...
jazyk OCL (Object Constraint Language). Pomocí tohoto jazyka je možné definovat
model to model transformaci.
...
Na předchozí stránce je vidět ukázka kódu transformace. Na začátku filu
...
Nejznámějším algoritmem používaným při porovnávání a zjišťování změn dvou
textových souborů je diff verzování souborů je nástroj diff\cite{diff_wiki}.
...
Delta notace vytvořená dle metamodelu na obr. \ref{fig:diff_cincetti} add,
delete entit a modify atributů jednotlivých entit, ale v našem modelu existují
operace, kterí se nedají zařadit ani do skupiny add, delete a modify operací,
...
nutné vybrat si správný algoritmus v závislosti na řešeném problému.
 ...
 modelu cílového. Existují 4 obecné skupiny dělení matching algoritmů. 
 ...
 \begin{itemize}
   \item Párování podle statického identifikátoru páruje elementy podle
 	...
 	nepodporujících maintenance unikátních identifikátorů.
 	...
 	vypočtení nestatické signatury jednotlivých features pomocí uživatelem
 	...
 	based matching podobnost features jednotlivých elementů, kterou agreguje do
 	skalární hodnoty. Tento princip se řadí mezi podtyp attribute graph matchingu.
...
\subsection{Graph matching}
 ...
 vzorky nalezeny v daných datech. 
 ...
 problémy týkající se shodnosti grafů vyjadřuje/znamená, že není možné nalézt
 ...
 V těchto případech není hledán izomorfismus dvou grafů, ale problém je upraven
 na hledání největší možné shody mezi grafy.
 Toto vede k třídě problémů známé jako inexact graph matching. V takovém
 případě hledáme nebijektivní korespondenci mezi grafem vzorku a grafem
...
\end{figure}
 ...
 \chapter{Vytvořený algoritmus rozpoznávání operací}

\section{Experimentální algoritmus}

V ranné fázi byl napsán prototyp jiného rozpoznávacího algoritmu, který se snaží 
minimalizovat vzdálenost současného modelu od modelu cílového pomocí
rozpoznáná operací a aplikace operací. Výhodou tohoto přístupu je nalezení více alternativních 
cest, nevýhodou je potom velikost stavového prostoru. Algoritmus prochází těmito fázemi:

\begin{enumerate}
  	\item Spočítání vzdálenosti vstupního modelu od modelu cílového
  	\item Nastavení nalezeného maxima zlepšujícího kroku na nula
  	\item Opakování následujícího cyklu dokud je rozpoznána jiná operace než je
  	Dummy operation a vzdálenost je větší než nula
  	\begin{enumerate}
  	  \item nastavení zlepšující operace na Dummy Operation
  	  \item  Pro každou operaci 
  	  	\begin{enumerate} 
    		\item nalezení nalezení nejvhodnějších parametrů operace
    		\item spočítání zlepšujícího kroku
    		\item aktualizace maxima zlepšujího kroku, podle
    		nalezených parametrů, aktualizace zlepšující operace
    	\end{enumerate}
    	\item aktualizace vzdálenosti
    	\item aplikace nejvýhodnější operace
    	\item nastavení maxima zlepšujícího kroku na nula
    \end{enumerate}
\end{enumerate}

Algoritmus předpokládá, že operace přibližující aktuální stav k cílovému by měly
být rozpoznány (a aplikovány) co nejdříve. Například pro počáteční model
zobrazený na obrázku \ref{fig:exp_contact} a cílový model zobrazený na obrázku
\ref{fig:exp_contact_address}. Algoritmus spočítá vzdálenost například
jako součet přidaných/odebraných tříd, primitivních atributů a asociačních
vazeb.
Vzdálenost těchto dvou modelů je tudíž 7 - 2 odebrané atributy z třídy Contact,
1 přidaná třída Address, 2 přidané atributy do třídy Address a 2 x vazba do tříd
Contact a Address (bidirectional vazby je nutné počítat za dvě asociace). 

Algoritmus v prvním průchodu rozpozná, že je možné aplikovat operace
AddClass(Address), RemoveProperty(Street) a ExtractClass(Address, Contact,
\{Street, City\}). AddClass zlepšuje vzdálenost o 1, RemoveProperty také o
jedna a ExtractClass o 7. Z těchto operací vyhodnotí jako nejlepší
ExtractClass a následně algoritmus skončí, protože po aplikaci se současný
model dostal do cílového stavu.
...
Přes počáteční slibné výsledky testované na konstruktivních operacích byl tento
algoritmus zavrhnut jako neefektivní. Algoritmus v každém kroku hledá parametry
i pro všechny operace, nedá se jednoduše analyzovat, pro které operace je nutné
hledat parametry znovu a které operace se již nedají rozpoznat. Druhou nevýhodou
je šířka prohledávaného stromu - algoritmus může nalézt víc operací, které
přinližují model stejnou měrou modelu cílovému, ale jakmile jednu z nich náhodně
vybere, zapomene veškeré mezivýsledky a v dalším kroku může rozpoznat všechny
operace z minulé iterace vyjma operace rozpoznané. V případě paralelizace by
prohledávaný prostor mohl exponenciálně růst. 

Nalezení vhodných vah jednotlivých elementů modelu a jejich atributů se dá
získat jedině metodou pokus-omyl a je nutné váhy znovu přehodnotit po přidání
nových operací. 

Experimentální algoritmus měl definován krok pro operace AddPrimitiveClass,
AddStandardClass, RenameEntity, RemoveEntity, RenameProperty. Problematickou
operací pro nastavování vah se stala operace ExtractClass.
 ...
 \section{První implementace}
 První jednodušší používá základu z UMLDiffu a očekává, že dvě třídy se stejným
 jménem jsou shodné, páruje tedy třídy podle jména. Následné rozdíly řeší rozpoznáním
 konstruktivních a destruktivních, případně některých operací modifikačních, ať
 už tyto operace pracovali s třídami nebo s property.
 
 První algoritmus pracuje podle tohoto pseudokódu:
 \begin{enumerate}
   \item Nastaví do seznamu SClses Standardní třídy z vstupního modelu  
   \item Nastaví do seznamu TClses Standardní třídy z cílového modelu  
   \item Nastaví do seznamu SProps Properties z vstupního modelu
   \item Nastaví do seznamu TProps Properties z cílového modelu
   \item seznam OPS :=\{\}
   \item Pro každou třídu S z vstupního modelu, která má stejné jméno jako třída
   T obsažená v modelu výstupním
   \begin{enumerate}
     \item Pro každou property $P_S$ z S, která má stejné jméno jako $P_T$
     Property z T
     \begin{enumerate}
       \item odstraň $P_S$ z SProps
       \item odstraň $P_T$ z TProps
     \end{enumerate}
     \item odstraň $S$ z SClses
     \item odstraň $T$ z TClses  
   \end{enumerate}
   \item Výsledný seznam SClasses transformuj na operace RemoveCls a přidej do
   seznamu Ops
   \item Výsledný seznam TClasses transformuj na operace AddCls a přidej do
   seznamu Ops
   \item Výsledný seznam SProps transformuj na operace RemoveProperty a přidej do
   seznamu Ops
   \item Výsledný seznam TProps transformuj na operace AddProperty a přidej do
   seznamu Ops
   \item Pro případně změněné atributy (isAbstract, parent, isUnique, isOrdered
  \ldots) přidej do seznamu Ops dané modifikační operace
 \end{enumerate}
 
 Tento algoritmus má schopnost rozeznat jen konstruktivní a destruktivní
 operace. Tudíž zachovvává data, jejichž identifikátor se nemění.
 Ostatní data naopak nekompromistně maže. Tento algoritmus není schopný
 rozpoznat operace RenameClass, RenameProperty, které jsou dle \cite{Luksch}
 nejčastěji používanými operacemi. Výhodou tohoto algoritmu je, že nepotřebuje
 definovat jiný diff nežli delta notaci.
 
 \section{Složitější implementace}
 ...
 Algoritmus rozpoznávání operací byl napsán se snahou o zachovávání co
 největšího množství dat. Ačkoliv triviální algoritmus pro přechod z modelu A k
 modelu B by mohl pomocí subtraktivních operací zničit model A a následně složit
 model B pomocí aditivních operací je zřejmé, že tento algoritmus neuchová žádná
 data.
...
\item[SŘBD] Systém řízení báze dat
\item[OCL] Object Constrain Language
\end{description}
\vdots
