...
rozpoznání operací a jejich následné aplikace. Výhodou tohoto přístupu
je možnost nalezení více alternativních cest, nevýhodou je potom velikost
stavového prostoru zvětšující se s počtem rozpoznávaných operací. Mezi operace
byla přidána takzvaná Dummy operation v případě, že nebyla rozpoznána
žádná operace. Algoritmus popsaný v pseudokódu viz %\ref{algo} 
\\
...
\Ensure Seznam operací $R_ops$, po jejichž aplikaci se model S tranformuje na
     model T
\Statex
\Procedure{Algoritmus procházení stavů}{$S,T$}
\State \textbf{Inicializace:}
%\State Vzdálenost modelů $D\gets$ dist(S, T)
\State $A \gets S$ \Comment {aktuální model}
\State $R\gets  DUMMY\_OPERATION$\Comment{Rozpoznaná operace}
\State $R_{ops} \gets \{\}$
\Statex
   \While{$R \neq NONE\_OPERATION$}\Comment{byla rozpoznána nějaká operace} 
   \State $R\gets NONE\_OPERATION$
   \State $K\gets 0$ \Comment{Zlepšující krok}
      \ForAll{op from Ops} \Comment{Pro každou operací z app metamodelu}
         \State $K_{op} \gets ZjistiZlepseni(op, A, T)$ \Comment{spočítá
         zlepšení}
      	\If{$K < K_{op}$}
      	   \State $K\gets K_{op}$
      	   \State $R \gets op$
      ...
      \EndFor
      \If{$R \neq NONE\_OPERATION$}
         \State $params \gets getParams(op, A, S)$
         \State $R_{ops}\gets R_{ops} +R(params)$
         \State apply(op, A)      
      \EndIf
   \EndWhile
\EndProcedure
...
%\end{algorithm}
\ \\
Algoritmus potřebuje ke svému běhu konfiguraci vah atributů jednotlivých
elementů modelu.
\begin{itemize}
   \item Váhy atributů třídy
   \begin{description}
      \item[classname] jméno třídy
      \item[parent] reference na rodičovskou třídu 
   \end{description}
   \item Váhy atributů všech property
   \begin{description}
      \item[propertyName] jméno property
      \item[type] reference na typ property
      \item[isOrdered] boolean značící ordered kolekci
      \item[isUnique] boolean značící unikátní kolekci
      \item[LowerBound + UpperBound] horní a dolní mez hodnot property
   \end{description}
   \item Váhy atributů asociačních property
   \begin{description}
      \item[oppositteProperty] reference na opozitní property u bidirectional
      vazby
   \end{description}
\end{itemize}
...
\begin{algorithmic}[1]
   \Require Zdrojový model S, cílový model T
   \Ensure Celočíselná hodnota vyjadřující vzdálenost modelu S od modelu T
   \Statex
   \Procedure{getDistance}{S, T}
   \State 
   \EndProcedure
\end{algorithmic}
\ \\
...
\begin{itemize}
      ...
      \item isOrdered = 1
      \item isUnique = 1
      \item LowerBound + UpperBound = 1
      \item oppositteProperty = 1
\end{itemize}

Na obrázcích vidíme \ref{fig:exp_contact} a cílový model
