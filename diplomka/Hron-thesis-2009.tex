...
$\setminus$ (množinový rozdíl) a $\times$ (kartézský součin) a operací selekce
...
V rámci Migdb bylo v posledních letech vytvořeny 4 bakalářské práce a 2
diplomové práce členů Migdb.
Jednalo se o práce mé osoby \cite{Lukes}, jež pojednávala o problematice mapování 
aplikačního modelu na model databázový práce Jiřího Ježka \cite{Jezek}
popisující aplikační model a jeho transformace, dále práce Petra Taranta
\cite{Tarant_bp} popisující databázový model a jeho transformace a poslední
...
byl často upravován, většinou zjednodušován.
...
Operace jsou uvedeny v tabulkách \ref{tab:opsSeznam1} a \ref{tab:opsSeznam2}. Kromě
regulérních operací, které může vytvořit uživatel jsou v tabulce
\ref{tab:opsSeznam2} uvedeny operace DistributeProperty, MergeProperty a
operace ExportProperty, které jsou používány jako pomocné v implementaci 
složitějších reduktivních a expanzivních operací a manipulují s Property v rámci dědičné struktury.
...
V \cite{Cincetti} autor definuje operace pomocí delta notace. Delta notace
...
difů $part_a part_b$,label=Man1_alternative] class Man { private String name;
...
že jakýkoliv patch je možné aplikovat na jakýkoliv model. Aplikace patche
\ref{Diff_1_2} na \ref{Man2} by vypadala \ref{Man2_p1_2}. Atribut byl přidán na
správné místo, ale konstruktor byl přidán na místo špatné - přídaná uzavírací
závorka neuzavírá předchozí konstruktor, ale celou třídu. Navzdory tomu, že
tento Java kód je nevalidní, patch bylo možné aplikovat.\\
...
navazující text diffu přidat, ale nemusí obsahovat, které řádky v druhém
souboru budou .
...
(intensional vs extensional - první potřebuje konkrétní model ke své aplikaci, 
druhá ne, extensional je možné použít na paralelní vývojové větve).

Tato notace mají operace některé specifické
vlastnosti jako je invertovatelnost a rozložitelnost. Je nutné říci, že operace
...
konstruktivními a  destruktivními operacemi. V projektu Migdb na druhé straně 
existují operace, které jsou kontextově závislé, což je uživatelskou
přívětivostí  operací - jako zástupcem takové operace se dá uvést operace 
AddParent(parentClass=B, childClass=A), která nezmiňuje všechny Property,  které
se mají odstranit, ale dynamicky si je dopočítává v závislosti na daném kontextu. 
Její inverzí je operace RemoveParent(childClass=A). Vzhledem k minimalističnosti
neexistuje k operaci RemoveParent(childClass=A) jednoznačná inverze bez daného
konktextu.
