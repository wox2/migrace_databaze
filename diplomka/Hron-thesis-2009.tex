...
\begin{algorithmic}[1]
   ...
   \State $R_{ops} \gets \{\}$
   \Statex	   
   \ForAll{$C_S \in S$} \label{algo:matching:forEqualCls}
      \ForAll{$C_T \in T$}
         \If{$C_T.name = C_S.name$}
   	        %\State markMatched(S, T, 'Equal') \Comment{Vytvoření equal páru}
   	        \State MATCH\_EQUAL\_PROPS(S, T) \Comment{Aktualizace matchovaných
   	        properties}
   ...
   \Procedure{match\_equal\_props}{clsSrcModel, clsTargetModel)}
   	   \ForAll{$P_S \in clsSrcModel$}
   	      \ForAll{$P_T \in clsTargetModel$}
...
Algoritmus inicializuje množiny nezpárovaných tříd z vstupního (a výstupního)
...
odstraní zpárované třídy z množin SClasses a TClasses a zpáruje jejich property
podle jména v proceduře MATCH\_EQUAL\_PROPERTIES. Procedura
MATCH\_EQUAL\_PROPERTIES odstraní v zpárovaných třídách property se stejným
...
\FloatBarrier
 ...
 EqualClassPair. V druhé fázi párování musí algoritmus párovat třídy podle
 jiného kritéria. Tímto mnou zvoleným kritériem je suma properties se shodným
 jménem. Čím vyšší je tato suma, tím jsou si třídy podobnější.\\
 
 Pro ilustraci si představme, že máme zdrojový model $M_1 = (Abcde)$ a cílový
 model $M_2 = (Abcf, Bdeg)$. Diff těchto dvou modelů je zobrazen v listings
 \ref{algo:match_exp:delta_exp}
 
 \begin{lstlisting}[frame=single,caption=Textový diff pro
 expanzivní operaci,label=algo:match_exp:delta_ext]
   +B            -Ade
   +Bdeg          
   +Af
\end{lstlisting}
 ...
 podobnosti 2, existují 2 property("e" a "f") ve třídě A ve zdrojovém modelu se
 ...
 Bdeg)$. Textový
  ...
  \begin{lstlisting}[frame=single,caption=Textový diff pro
 reduktivní operaci,label=algo:match_exp:delta_red]
   +Ade          -B            
                 -Bdeg          
                 -Af
\end{lstlisting}
 ...
 Shodné třídy a třídy přejmenované potom tvoří pilíře pro operace reduktivní a
 expanzivní, které se vážou na rozpoznané páry.
...
\subsection{Diff elementy}
 Základní párovací algoritmus pároval třídy podle jména
 ...
 nejd, byla přidána třída ReplacingClassPair - nahrazující pár, který se používá jako pivot pro hledání expansivních, 
