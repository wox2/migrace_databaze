 ...
 * Okno voleb pro program Piškorky 2.1
 ...
 * Vylepšeno o filtr v oknì pro volbu souboru
 */
...
{                                             // Tlaèítek je vždy n x n
  ...
  // Vnitøní tøída (její deklarace je vnoøena do tøídy OknoVoleb)
  // ** Tøída filtru nemusí být vnitøní **
  //
  // Tøída filtru pro výbìr souborù zobrazovaných v dialogovém oknì JFileChooser
  // Musí být potomkem tøídy javax.swing.filechooser.FileFilter
  // a musí implementovat veøejnì pøístupnou metodu
  // boolean accept(File f),
  // která rozhoduje, zda se soubor zobrazí
  // Metoda
  // public String getDescription()
  // vrátí popis, který se zobrazí v øádce pod jménem souboru (Files of type)
  class Filtr extends javax.swing.filechooser.FileFilter
  {
    private String pripony = ".jpg .png .jpeg .gif"; // Akceptované pøípony
    ...
    private String getExtension(File soubor)  // Získá pøíponu souboru
    {
      if(soubor != null)
      {
        String jmeno = soubor.getName();      // Získá celé jméno
        int i = jmeno.lastIndexOf('.');       // Teèkou zaèíná pøípona
        if (i > 0 && i < jmeno.length()-1)    // Vyjmi pøíponu a pøeveï ji na malá písmena
         return jmeno.substring(i+1).toLowerCase();// zkus ji najít v øetìzci pripony
      }
      return "";                // Když tam není, vra prázdný øetìzec
    }
    public String getDescription() // Vrátí popis
    {
      return "Grafické soubory pro Javu";
    }
  }
  ...
  void volba_Akce(ActionEvent e){
   JTextField pole = e.getSource().equals(volic1)? obrazek1:obrazek2;
    ...
    vyberSoubor.setDialogTitle("Nový symbol hráèe");
