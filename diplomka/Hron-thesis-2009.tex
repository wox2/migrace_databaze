...
dokončení projektu Migdb, aby byl implementovaný framework provozuschopný,
tomuto tématu se věnuje kapitola \ref{chapt:dokonceni_projektu}, přičemž kapitola
\ref{chapt:projekt_migdb} popisuje projekt Migdb, jeho historii a
dosažené výsledky před započetím této diplomové práce. Druhým tématem je
automatické rozpoznávání operací nad aplikačním modelem. Tomuto tématu se věnuje
kapitola \ref{chapt:recognition}, implementaci rozpoznávacích algoritmů se potom
věnuje kapitola \ref{chapt:recognition_implementation}. V kapitole
\ref{chapt:testování} jsou shrnuty způsoby otestování jednotlivých částí
implementovaného frameworku a jsou zhodnoceny jejich výstupy. Kapitola
\ref{chapt:zaver} bilancuje dosažené výsledky této diplomové práce a navrhuje
další možné směry k rozvíjení daných dvou témat.
...
databázového modelu. Množinu stavů, kterými databázový model prochází potom
...
sekvenční představě viz popis \ref{framework:jezek_def} iteračně viz obr.
\ref{fig:framework:modified}.
...
ostatních modulech. Je mu věnována kapitola rozpoznávání operací
\ref{chapt:recognition}. \\
...
seznam aplikačních operací a seznam Diff elementů. Pro každou část je
...
jeden container - potomka třídy ModelRoot.
...
Každá ModelEntita obsahuje svůj identifikátor name. Primitivní typy
programovacího jazyka jsou reprezentované elementem \textit{PrimitiveClass}
potomkem ModelEntity. První potomek ModelEntity, PrimitiveClass obsahuje jen
\textit{primitiveType}.
Druhým potomkem ModelEntity je \textit{StandardClass} a obsahuje specifikaci
svého \textit{inheritancetType}, příznak \textit{isAbstract}, referenci na
...
vazby či vymezují vlastnosti primitivní Property, pro kolekce důležité atributy
\textit{isUnique} a \textit{isUnique}. \textit{PrimitiveProperty}, potomek
Property, rozšiřuje svou rodičovskou třídu jen o svůj \textit{type}, který musí
být primitivní.
...
bidirectional vazby a atribut isOwning, který existuje z implementačních
důvodů více o něm bude zmíněno v kapitole \ref{chapt:dokonceni_projektu}. \\
...
\section{Metamodel struktury databáze}
 ...
 identifikované svým \textit{name}, obsahuje sezname tabulek a seznam Sequencí.
 Sekvence je databázový element potřebný k postupnému generování čísel a je
...
zpřístupňující Constraint.  TableConstraint \textit{Unique} obsahuje seznam
unikátních sloupců a odkaz na vlastnickou tabulku \textit{ownintTable}.
...
omezen počet \textit{constrainedColumns} na 1 a obsahuje také odkaz na
referencovanou tabulku. Na obr. \ref{fig:rdb_str} je zobrazen aktuální
...
První metodou je metoda boolean isValid(Structure structure), která pro danou
aplikační strukturu zjistí, jestli je daná operace proveditelná nad touto
strukturou, a vrátí tuto informaci ve své náhratové hodnotě. Druhá metoda
void apply(Structure structure) aplikuje operaci na danou strukturu, tj. pozmění
elementy v ní obsažené.
...
\begin{list}{Operace \Roman{ListCitac}:}{\usecounter{ListCitac}}
  ...
  \begin{itemize}
    \item Validační podmínky - existuje třída s daným jménem, neexistuje link
    na tuto třídu, třída neobsahuje žádné property, neexistuje pro tuto třídu
    žádný potomek
  ...
  \begin{itemize}
    ...
    virtuální operaci distribute property) property z rodičovské třídy do třídy
    původního potomka
  \end{itemize}
...
mapuje jeden vstupní model \'source\' typu ABC na jeden výstupní model
\'target\' typu ABC. Kromě vstupních a výstupních elementů mohou být v
...
\textit{self.elements[A]} a je výsledek volání mapování A2B() je přidán do
kolekce elements výstupní entity Root.

Mapování A2B() mapuje entity typu A na elementy typu B. podmínkou v bloku when
určuje doplňující podmínku, budou se mapovat jen entity typu A s id > 0.
Mapování nastaví výsledné entitě id vstupní entity a rozšíří text uložený v
entitě o řetězec \textit{World!}. 

Ukázkový kód bude tedy pro ukázková data 
\begin{verbatim}
   A: id=1, A=”Hello”
   A: id=-1, A=”World”
   C: id=1, C=”Something”
...
dotazovací konstrukt query a modifikační konstrukt helper. V následujícím kódu
je ukázáno validační query pro zjištění splnitelnosti validačních podmínek
operace AddTable. Query isValid převolává  další dvě query. V helperu
checkExistSchema vidíme kontrolu a případné zalogování chyby při nenalezení
schématu, Není tedy možné vracet obrácenou hodnotu volání pokud chceme zjistit,
že daná validační podmínka neplatí, protože by tímto voláním byla zalogována chyba.
...
\subsection{Změny v aplikačním metamodelu}
\begin{figure}[h]
\begin{center}
...
po odstranění kontrolního mechanismu snaha o odstranění kontrolních queries,
...
\caption{Stav v průběhu vývoje aplikace}
...
\subsection{Změny evouluce aplikačního modelu}

\section{Databázová vrstva}
\subsection{Změny Databázového metamodelu}
...
\subsection{Změny databázových operací}
