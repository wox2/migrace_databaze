...
#define Z 2
  ...
  for (int iz = 0; iz < _resZ; iz++) {
	...
	for (int ix = 0; ix < _resX; ix++) {
		...
		if(count == 0) {
			char errorStr[256];
			sprintf(errorStr, "ModelManager::LoadTerrain(): Failed to find any normals at: [%d, %d]", ix, iz);
...
#undef Z
  ...
  subMesh_p->ambient[0] = 0.7f;
  subMesh_p->ambient[1] = 0.7f;
  subMesh_p->ambient[2] = 0.7f;
