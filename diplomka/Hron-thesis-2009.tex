...
Acsa)$ vzdálenost těchto dvou modelů vyjádřit pomocí
 delta notace zobrazené v listings \ref{algo:state:delta_example}.
...
Všechny atributy jsou vyjmenovány v následujícím seznamu.
...
\begin{itemize}
      ...
      \item isOrdered = 0
      \item isUnique = 0
      \item LowerBound + UpperBound = 0
      \item oppositteProperty = 0
\end{itemize}

Tato konfigurace redukuje získávání energie na původně zmiňovaný způsob definice
Enerigie.

Další možnou modifikací je úprava seznamu procházených operací. Algoritmus je
schopný rozpoznat jednotlivé operace, ale může mít problémy s rozpoznáním
...
rozšíření seznamu $R_{ops}$ o dvojici operací RemoveProperty a AddProperty, je
nutné a nastavit získávání zlepšujícího kroku pouze pro případy, kdy existuje
property, která má v výstupním modelu jiný typ než ve vstupním.
...
hledat parametry znovu, kterých operací se aplikace rozpoznané operace nedotkla 
a které operace se již po aplikaci nedají rozpoznat.
Druhou nevýhodou je šířka prohledávaného stromu - algoritmus může nalézt víc operací, které
