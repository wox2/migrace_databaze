...
void initializeScene()
{
  ...
  // vytvorime transformNode a diky rotaci mu inicializujeme puvodni natoceni
  ...
  // ted ho posuneme a zmenime velikosti
  jeep_transform->translate(glm::vec3(0.0,0.0,-8.0));
  jeep_transform->scale(glm::vec3(4.0,4.0,4.0));


  RotationAnimNode * jeep_rotace = new RotationAnimNode("jeep_rotace", jeep_transform);
  // nastavime mu rotacni uzel, bude se otacet podle osy Z s rychlosti 0.5
  jeep_rotace->setAxis(glm::vec3(0, 0, 1)); 
  jeep_rotace->setSpeed(0.5); 
  ...
  MeshNode* jeep_mesh_p = new MeshNode(JEEP_FILE_NAME, jeep_rotace );
 ...
 // =============================== BEGIN OF SOLUTION - TASK 1,3 ======================================
  ...
  // nastavime elipseAnimNode, jako rodicovsky uzel bude root_node
   EllipseAnimNode * cessna_rotace2 = new EllipseAnimNode("cessna_rotace2", root_node);

   cessna_rotace2->setAxes(A,B); // elipsa bude mit poloosy A,B
   cessna_rotace2->setSpeed(0.5);  

   // nastavime transformNode s rodicovskym uzlem cessna_rotace2
   TransformNode* cessna_transformace2 = new TransformNode("cessna_transformace2", cessna_rotace2);

   // ted pouze cessnu zvetsime a staticky natocime (toto natoceni bude vzdy stejne pred aplikaci ellipseAnimNode)
                 cessna_transformace2 ->scale(glm::vec3(8.0, 8.0, 8.0));
                cessna_transformace2 ->rotate(90.0, glm::vec3(1.0, 0.0, 0.0));


      MeshGeometry* meshGeom_cessna2 = NULL;
  if((meshGeom_cessna2 = ModelManager::Instance()->Get(CESSNA_FILE_NAME)) == NULL) {
      meshGeom_cessna2 = ModelManager::Instance()->Load(CESSNA_FILE_NAME);
  }

  // nastaveni geometrie
  MeshNode * cessna_mesh_p2 = new MeshNode(CESSNA_FILE_NAME, cessna_transformace2 );
  cessna_mesh_p2->setGeometry(meshGeom_cessna2);
 ...
 // =============================== END OF SOLUTION - TASK 1,3 ======================================
  ...
  // =============================== BEGIN OF SOLUTION - TASK 1,3 ======================================
   ...
   // nastavime rotationAnimNode cessne s rodicovskym uzlem root_node

   RotationAnimNode * cessna_rotace = new RotationAnimNode("cessna_rotace", root_node);

   // otaceni podle osy Y
   cessna_rotace->setAxis(glm::vec3(0, 1, 0)); 
   cessna_rotace->setSpeed(0.5); 

   // cesnsu posuneme na prvotni pozici (15,0,0), zvetsime ji a natocime (aby nebyla nakrivo)
   TransformNode* cessna_transformace = new TransformNode("cessna_transformace", cessna_rotace);
   				 cessna_transformace ->translate( glm::vec3(15, 0.0, 0.0 ));
                 cessna_transformace ->scale(glm::vec3(8.0, 8.0, 8.0));
                cessna_transformace ->rotate(267, glm::vec3(0.0, 1.0, 0.0));
                cessna_transformace ->rotate(90.0, glm::vec3(1.0, 0.0, 0.0));


      MeshGeometry* meshGeom_cessna = NULL;
  if((meshGeom_cessna = ModelManager::Instance()->Get(CESSNA_FILE_NAME)) == NULL) {
      meshGeom_cessna = ModelManager::Instance()->Load(CESSNA_FILE_NAME);
  }

  MeshNode * cessna_mesh_p = new MeshNode(CESSNA_FILE_NAME, cessna_transformace );
  cessna_mesh_p->setGeometry(meshGeom_cessna);
