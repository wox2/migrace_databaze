...
hodnotu aktuálního nejlepšího zlepšujícího kroku K na 0, aby zajistil přemazání
dat nalezené operace v minulé iteraci cyklu. 
...
Základní myšlenkou pro vytvoření algoritmu byla vzdálenost modelu od prázdného
modelu. Tuto základní vzdálenost můžeme nazvat energií modelu. Pokud se zaměříme
na samotnou existenci tříd s daným jménem a properties s daným jménem v modelu
a pomineme ostatní atributy properties a tříd můžeme si představit energii
modelu jako součet existujících tříd a jejich properties.
...
\Ensure Seznam operací $R_{ops}$, po jejichž aplikaci se model S tranformuje na
     model T
\Statex
...
Na listing \ref{algo:state:delta} máme na levé straně zobrazenou
...
Přičemž $E(PRIM_{XY})$ reprezentuje energii změněných primitivních atributů:
$$E(PRIM_{XY}) = \sum_{A: (A.owner \notin M_{X} \vee A.owner \notin M_{Y})
\vee A_{X} != A_{Y} }E(A)$$
...
\begin{lstlisting}[mathescape,frame=single,caption=Textový diff
modelů pro ukázkový příklad,label=algo:state:delta_example]
   +C$c_{on}$    -Csc
   +A            
   +Asca
\end{lstlisting}
